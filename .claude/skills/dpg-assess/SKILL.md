...
1. **Run the audit.** From this repo's root run:
   ```bash
   python3 scripts/audit/run_audit.py <TARGET_REPO> --type <TYPE> -o /tmp/dpg-audit-report.json
   ```
   Then read `/tmp/dpg-audit-report.json`. (If `scripts/audit/run_audit.py` is not reachable because you are assessing a different repo, run it from this scaffolder repo's location.) Note which tools were `not_run` — the report says so under `tools_available` — and surface that as a confidence caveat.

2. **Load the knowledge core.** Read `maturity/indicators.md` (what each indicator requires + how to detect it) and `maturity/tier-model.md` (the tier ladder + classification algorithm). Read `.claude/skills/dpg-assess/rubric.md` for the scoring rules.
